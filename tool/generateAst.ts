...
function defineVisitor(lines: string[], baseName: string, types: string[]) {
  ...
  for (const type of types) {
    ...
    lines.push(`  visit${typeName}${baseName}(${baseName.toLowerCase()}: ${typeName}${baseName}): R`)
  }
  lines.push('}')
  lines.push('')
} 

function defineType(lines: string[], baseName: string, className: string, fieldList: string) {
